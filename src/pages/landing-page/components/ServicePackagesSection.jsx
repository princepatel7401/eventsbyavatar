        ...
        <div className="text-center mb-8 sm:mb-16">
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-display font-bold text-text-primary mb-4 sm:mb-6">
            ...
            <span className="block text-primary">
              Clear Value, Transparent Pricing
            </span>
          </h2>
          <p className="text-base sm:text-lg md:text-xl text-text-secondary max-w-3xl mx-auto leading-relaxed">
            Choose the perfect package for your event size and requirements. All
            prices in lakhs notation with transparent inclusions.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mb-8 sm:mb-12">
          {packages.map((pkg, index) => (
            ...
            >
              ...
              {pkg.badge && (
                <div
                  className={`absolute -top-3 sm:-top-4 left-1/2 transform -translate-x-1/2 px-4 sm:px-6 py-1 sm:py-2 rounded-full text-xs sm:text-sm font-semibold text-white ${
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-custom-lg p-4 sm:p-8 border border-border">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              {Object.entries(packages[selectedPackage].inclusions).map(
                ([category, items]) => (
                  <div
                    key={category}
                  ...
                  >
                    <button
                      onClick={() =>
                        toggleInclusionExpansion(selectedPackage, category)
                      }
        ...
        <div className="mt-8 sm:mt-12 grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-custom border border-border text-center">
            <Icon
              name="Shield"
              size={24}
              className="sm:w-8 sm:h-8 text-primary mx-auto mb-2 sm:mb-4"
            />
          ...
          <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-custom border border-border text-center">
            <Icon
              name="Clock"
              size={24}
              className="sm:w-8 sm:h-8 text-accent mx-auto mb-2 sm:mb-4"
            />
            <h4 className="font-semibold text-text-primary mb-1 sm:mb-2 text-sm sm:text-base">
              Flexible Payments
            </h4>
            <p className="text-text-secondary text-xs sm:text-sm">
              Pay in installments as per event timeline
            </p>
          </div>
          ...
          <div className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-custom border border-border text-center">
            <Icon
              name="Users"
              size={24}
              className="sm:w-8 sm:h-8 text-success mx-auto mb-2 sm:mb-4"
            />
            <h4 className="font-semibold text-text-primary mb-1 sm:mb-2 text-sm sm:text-base">
              Dedicated Team
            </h4>
            <p className="text-text-secondary text-xs sm:text-sm">
              Personal project manager for every event
            </p>
          </div>
        </div>
      </div>
